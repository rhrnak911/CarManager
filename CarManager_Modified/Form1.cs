...
selectQuery();
        ...
        private void ConnectDB()
        {
            conn.ConnectionString = string.Format("Data Source={0}; " +
                    "Initial Catalog = {1};" +
                    "Persist Security Info = {2};" + // 정보 보안 유지
                    "User ID = {3};"+
                    "Password={4}"
                    , "192.168.0.200", "2020DB", "True", "sa", "8765432!");
            /*
             * string strconn =
           "Data Source=192.168.0.200;Initial Catalog=2020DB;Persist Security Info=True;User ID=sa;Password=8765432!";
        ...
        private void insertQuery(int parkingSpot)
        {
            ...
            try
            {
                ...
                cmd.Connection = conn;
                //cmd.CommandType = CommandType.StoredProcedureText;
            ...
            {
                WriteLog(e.Message + Environment.NewLine + e.StackTrace);
            }
        }
        ...
        private void button_parkingAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    int parkingSpot = 0;
                    int.TryParse(textBox_parkingSpot.Text, out parkingSpot);
                    if (parkingSpot <= 0)
                    {
